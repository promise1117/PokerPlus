...
class UcenterController extends \Home\Controller\BaseController
{
	...
	public function modifyUserInfo()
	{
		...
		$str = substr(md5(time()), 0, 10);//md5加密，time()当前时间戳

		if (!empty($avatar) && !empty($img_ext)) {
			$file_data = base64_urlsafedecode($avatar);

			$file_name = './Public/Uploads/avatar/' . $user_id . '_a_b_'.$str.'.'.$img_ext;
...
//			$file_data = base64_decode(substr(strstr($avatar_thumb,','),1));
			$file_name = './Public/Uploads/avatar_thumb/' . $user_id . '_a_s_'.$str.'.'.$img_ext;
			$ret = file_put_contents($file_name, $file_data);
...
//			$data['avatar'] = $avatar_name;
			$data['avatar'] = $user_id . '_a_b_'.$str.'.'.$img_ext;
//			$data['avatar_thumb'] = $thumb_name;
			$data['avatar_thumb'] = $user_id . '_a_s_'.$str.'.'.$img_ext;
		}else{
			$data['avatar'] = 'default.jpg';
			$data['avatar_thumb'] = 'default.jpg';
		}
...
//		}
		if (!empty($avatar) && !empty($avatar_thumb)) {
			$data['avatar'] = 'http://' . c('SERVER_DOMAIN') . '/Public/Uploads/avatar/' . $user_id . '_a_b_'.$str.'.'.$img_ext;
			$data['avatar_thumb'] = 'http://' . c('SERVER_DOMAIN') . '/Public/Uploads/avatar_thumb/' . $user_id . '_a_s_'.$str.'.'.$img_ext;
			$data['user_marks'] = $user_marks;
		}else{
			$data['avatar'] = 'http://' . c('SERVER_DOMAIN') . '/Public/Uploads/avatar/default.'.$img_ext;
			$data['avatar_thumb'] = 'http://' . c('SERVER_DOMAIN') . '/Public/Uploads/avatar_thumb/default.'.$img_ext;
			$data['user_marks'] = $user_marks;
		}
...
//		$result['msg'] = $avatar;
	...
	public function sendVcodeByUnbindMobile()
	{
		...
		$res = $this->sendSmsNew($parse_mobile, $msg);
...
//		$qiniu = c('QINIU');
		...
		if (!empty($res['avatar'])) {
			if(!file_exists('./Public/Uploads/avatar/'.$res['avatar'])){
				$res['avatar'] = 'http://' . c('SERVER_DOMAIN') . '/Public/Uploads/avatar/default.jpg';
			}else{
				$res['avatar'] = 'http://' . c('SERVER_DOMAIN') . '/Public/Uploads/avatar/'.$res['avatar'];
			}
...
//			$res['avatar'] = 'http://' . $qiniu['domain'] . '/' . $res['avatar'];
		...
		if (!empty($res['avatar_thumb'])) {
			if(!file_exists('./Public/Uploads/avatar_thumb/'.$res['avatar_thumb'])){
				$res['avatar_thumb'] = 'http://' . c('SERVER_DOMAIN') . '/Public/Uploads/avatar_thumb/default.jpg';
			}else{
				$res['avatar_thumb'] = 'http://' . c('SERVER_DOMAIN') . '/Public/Uploads/avatar_thumb/'.$res['avatar_thumb'];
			}
