...
class RegisterController extends \Home\Controller\BaseController
{
	...
	public function userAdd()
	{
		...
		//调用vg钱包注册新用户，返回uid并写入user_main表中
		$vg_register = R('User/Vg/register',array(
				'loginNo'=>strval($mobile),
				'userName'=>'',
				'nickName'=>'pp'.substr(md5(time()),0,6),
				'countryCode'=>'86',
				'password'=>strval($vg_passwd),
			));
		$vg_register = json_decode($vg_register,true);
		if($vg_register['result']){
			$vg_uid = $vg_register['result']['uid'];
		}else{
			$vg_uid = '';
		}
		$data = array('mobile' => $mobile, 'passwd' => $passwd, 'diamond_num' => $user_init['diamond_num'], 'user_gold' => $user_init['user_gold'], 'games_max' => $user_init['games_max'], 'clubs_max' => $user_init['clubs_max'], 'current_games' => $user_init['current_games'], 'current_clubs' => $user_init['current_clubs'], 'reg_code' => strtolower(randstrcode(6)), 'sale_code' => strtolower(randstrcode(6, 'NUMBER')), 'card_type' => $user_init['card_type'], 'card_expire' => time() + $user_init['card_expire'], 'token' => $token, 'reg_time' => $reg_time, 'last_login' => $last_login, 'is_online' => 1,'vg_uid'=>$vg_uid);
